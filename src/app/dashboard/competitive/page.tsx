...
interface CompetitorApiData {
  ...
  marketShareHistory: Record<string, string | number>[];
  hasRealMarketShare: boolean;
...
export default function CompetitivePage() {
  ...
  // Mock fallback for market share chart
  const { data: mockShareData } = useApiData(
    '__mock__',
    () => generateCompetitorData(900),
  );

  // Use real market share history if available, mock fallback
  const shareData = useMemo(() => {
    if (apiData?.marketShareHistory && apiData.marketShareHistory.length > 0) {
      return apiData.marketShareHistory as { date: string; [key: string]: string | number }[];
    }
    return mockShareData;
  }, [apiData?.marketShareHistory, mockShareData]);

  const isMarketShareSimulated = !apiData?.hasRealMarketShare;
  ...
  return (
    <div className="space-y-4">
      <MarketShareChart data={shareData} isSimulated={isMarketShareSimulated} />
